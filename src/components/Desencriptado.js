...
export const Desencriptado = () => {
	...
	const { encoded, encrypted, key, decoded, decrypted, setDecoded, setDecrypted } = useContext(ViewContext)

	const handleDecoded = useCallback((decrypted) => {
		const utf8 = CryptoJS.enc.Base64.parse(decrypted)
		const textPlain = CryptoJS.enc.Utf8.stringify(utf8)
		setDecoded(textPlain)
	}, [setDecoded])

	const handleDecrypted = useCallback((encrypted, key) => {
